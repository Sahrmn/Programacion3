			...
			{						
				...
				{		
					...
					*/$response = $next($request, $response);
					...
					if($payload->perfil == 'administradorss')
					{
						$response = $next($request, $response);
						return $response;
					}
					else
					{
						...
						//var_dump($arrayRecibido[0]->titulo);
						$nuevoArray = array();
						for ($i=0; $i < count($arrayRecibido); $i++) { 
							$nuevoArray[$i] = $arrayRecibido[$i]->titulo;
						}
						//$objDelaRespuesta->respuesta = "no es admin";
						//$nueva = $response->withJson($objDelaRespuesta, 401);  
						$nueva = $response->withJson($nuevoArray, 200);
						return $nueva;
					}
					//$response = $next($request, $response);
